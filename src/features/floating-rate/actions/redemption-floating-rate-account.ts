...
/**
 * Process a redemption from a floating rate investment account
 */
export async function redeemFloatingRateAccount(
  ...
  try {
    // Get account details with floating rate info
    ...
    const currentValue = await calculateFloatingRateValueWithRedemptions(
      request.accountId,
      netInvestorFund,
    ...
    const mutationResult = await db
      ...
      .values({
        ...
        description:
          request.description ||
          `Redemption from floating rate account ${account.accountNumber}`,
        status: "completed",
    ...
    console.error("Floating rate redemption error:", error);
    return {
      success: false,
...
/**
 * Get all redemption history for a floating rate account
 */
export async function getFloatingRateAccountRedemptions(
  accountId: number
