...
export async function calculateNetPresentValueWithRedemptions(
  ...
  while (calculationDate <= endDate) {
    ...
    if (isFullyRedeemed) {
      console.log(
        `Account fully redeemed in ${monthKey}. Stopping calculation.`
      );
...
/**
 * Simplified version for client-side use (without database access)
 */
export function calculateNetPresentValueWithKnownRedemptions(
  grossCapital: number,
  annualRate: number,
  startDate: Date,
  currentDate: Date = new Date(),
  ...
  while (calculationDate <= endDate) {
    ...
    // Check if account is fully redeemed
    const isFullyRedeemed =
      balanceAfterRedemptions < 1000 && redemptionsThisMonth > 0;
    ...
    if (isFullyRedeemed) {
      console.log(
        `Client: Account fully redeemed in ${monthKey}. Stopping calculation.`
      );
      currentValue = 0; // Set final value to 0 for fully redeemed accounts
      break;
    }
